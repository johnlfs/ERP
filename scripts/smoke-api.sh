...
CANCELED_ACCOUNT_RECEIVABLE_CASH_MOVEMENTS_RESPONSE="$(json_request GET "/api/v1/cash-movements?accountReceivableId=${ACCOUNT_RECEIVABLE_ID}&page=1&pageSize=10" "200" "" )"

CANCELED_ACCOUNT_RECEIVABLE_CASH_MOVEMENTS_RESPONSE="$CANCELED_ACCOUNT_RECEIVABLE_CASH_MOVEMENTS_RESPONSE" ACCOUNT_RECEIVABLE_ID="$ACCOUNT_RECEIVABLE_ID" python3 - <<'PYVALIDATION'
import json
import os

movements = json.loads(os.environ["CANCELED_ACCOUNT_RECEIVABLE_CASH_MOVEMENTS_RESPONSE"])["data"]
assert len(movements) == 0, (
    f"Conta a receber cancelada não deveria gerar cash movement, veio {len(movements)}"
)
PYVALIDATION
...
CANCELED_ACCOUNT_PAYABLE_CASH_MOVEMENTS_RESPONSE="$(json_request GET "/api/v1/cash-movements?accountPayableId=${ACCOUNT_PAYABLE_ID}&page=1&pageSize=10" "200" "" )"

CANCELED_ACCOUNT_PAYABLE_CASH_MOVEMENTS_RESPONSE="$CANCELED_ACCOUNT_PAYABLE_CASH_MOVEMENTS_RESPONSE" ACCOUNT_PAYABLE_ID="$ACCOUNT_PAYABLE_ID" python3 - <<'PYVALIDATION'
import json
import os

movements = json.loads(os.environ["CANCELED_ACCOUNT_PAYABLE_CASH_MOVEMENTS_RESPONSE"])["data"]
assert len(movements) == 0, (
    f"Conta a pagar cancelada não deveria gerar cash movement, veio {len(movements)}"
)
PYVALIDATION
...
PAID_CASH_MOVEMENTS_OUTSIDE_OCCURRED_AT_RESPONSE="$(json_request GET "/api/v1/cash-movements?occurredAtFrom=2030-03-03T00:00:00.000Z&occurredAtTo=2030-03-04T00:00:00.000Z&accountPayableId=${PAID_ACCOUNT_PAYABLE_ID}&page=1&pageSize=10" "200" "" )"
PAID_CASH_MOVEMENTS_BY_SEARCH_RESPONSE="$(json_request GET "/api/v1/cash-movements?search=Baixa&accountPayableId=${PAID_ACCOUNT_PAYABLE_ID}&page=1&pageSize=10" "200" "" )"
...
paid_cash_outside_occurred_at = json.loads(os.environ["PAID_CASH_MOVEMENTS_OUTSIDE_OCCURRED_AT_RESPONSE"])["data"]
paid_cash_by_search = json.loads(os.environ["PAID_CASH_MOVEMENTS_BY_SEARCH_RESPONSE"])["data"]
...
assert len(paid_cash_outside_occurred_at) == 0, (
    f"Filtro cash occurredAt fora da baixa deveria retornar 0, veio {len(paid_cash_outside_occurred_at)}"
)
assert any(movement["id"] == paid_cash_movement_id for movement in paid_cash_by_search), (
    "Busca de cash movement por descrição/notas deveria encontrar a saída da baixa"
)
PYVALIDATION

json_request PATCH "/api/v1/accounts-payable/${PAID_ACCOUNT_PAYABLE_ID}/pay" "400" "$PAY_ACCOUNT_PAYABLE_PAYLOAD" >/dev/null
PAID_CASH_MOVEMENTS_AFTER_DUPLICATE_PAY_RESPONSE="$(json_request GET "/api/v1/cash-movements?accountPayableId=${PAID_ACCOUNT_PAYABLE_ID}&page=1&pageSize=10" "200" "" )"
PAID_CASH_MOVEMENTS_AFTER_DUPLICATE_PAY_RESPONSE="$PAID_CASH_MOVEMENTS_AFTER_DUPLICATE_PAY_RESPONSE" PAID_CASH_MOVEMENT_ID="$PAID_CASH_MOVEMENT_ID" python3 - <<'PYVALIDATION'
import json
import os

movements = json.loads(os.environ["PAID_CASH_MOVEMENTS_AFTER_DUPLICATE_PAY_RESPONSE"])["data"]
assert len(movements) == 1, f"Segunda baixa não deveria criar novo cash movement, veio {len(movements)}"
assert movements[0]["id"] == os.environ["PAID_CASH_MOVEMENT_ID"], "Cash movement após segunda baixa deveria permanecer o mesmo"
PYVALIDATION
...
RECEIVED_CASH_MOVEMENTS_OUTSIDE_OCCURRED_AT_RESPONSE="$(json_request GET "/api/v1/cash-movements?occurredAtFrom=2030-05-03T00:00:00.000Z&occurredAtTo=2030-05-04T00:00:00.000Z&accountReceivableId=${RECEIVED_ACCOUNT_RECEIVABLE_ID}&page=1&pageSize=10" "200" "" )"
RECEIVED_CASH_MOVEMENTS_BY_SEARCH_RESPONSE="$(json_request GET "/api/v1/cash-movements?search=Recebimento&accountReceivableId=${RECEIVED_ACCOUNT_RECEIVABLE_ID}&page=1&pageSize=10" "200" "" )"
...
received_cash_outside_occurred_at = json.loads(os.environ["RECEIVED_CASH_MOVEMENTS_OUTSIDE_OCCURRED_AT_RESPONSE"])["data"]
received_cash_by_search = json.loads(os.environ["RECEIVED_CASH_MOVEMENTS_BY_SEARCH_RESPONSE"])["data"]
...
assert len(received_cash_outside_occurred_at) == 0, (
    f"Filtro cash occurredAt fora do recebimento deveria retornar 0, veio {len(received_cash_outside_occurred_at)}"
)
assert any(movement["id"] == received_cash_movement_id for movement in received_cash_by_search), (
    "Busca de cash movement por descrição/notas deveria encontrar a entrada do recebimento"
)
...
RECEIVED_CASH_MOVEMENTS_AFTER_DUPLICATE_RECEIVE_RESPONSE="$(json_request GET "/api/v1/cash-movements?accountReceivableId=${RECEIVED_ACCOUNT_RECEIVABLE_ID}&page=1&pageSize=10" "200" "" )"
RECEIVED_CASH_MOVEMENTS_AFTER_DUPLICATE_RECEIVE_RESPONSE="$RECEIVED_CASH_MOVEMENTS_AFTER_DUPLICATE_RECEIVE_RESPONSE" RECEIVED_CASH_MOVEMENT_ID="$RECEIVED_CASH_MOVEMENT_ID" python3 - <<'PYVALIDATION'
import json
import os

movements = json.loads(os.environ["RECEIVED_CASH_MOVEMENTS_AFTER_DUPLICATE_RECEIVE_RESPONSE"])["data"]
assert len(movements) == 1, f"Segundo recebimento não deveria criar novo cash movement, veio {len(movements)}"
